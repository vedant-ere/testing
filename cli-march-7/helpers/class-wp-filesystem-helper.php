...
class Wp_Filesystem_Helper {
	...
	/**
	 * Write one CSV row to file.
	 *
	 * @param string       $path File path.
	 * @param array<mixed> $row CSV row values.
	 * @param bool         $append Whether to append or truncate/write.
	 * @return void
	 * @throws \RuntimeException When row write fails.
	 */
	public function write_csv_row( string $path, array $row, bool $append = true ): void {
		$mode = $append ? 'ab' : 'wb';

		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- Required for streaming CSV writes in WP-CLI export.
		$handle = fopen( $path, $mode );
		if ( false === $handle ) {
			throw new \RuntimeException(
				sprintf(
					/* translators: %s: file path. */
					__( 'Failed to open CSV file: %s', 'rt-movie-library' ),
					(string) $path
				)
			);
		}
		...
		try {
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fputcsv,WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_fwrite -- Intentional: use native CSV escaping/quoting and stream one row at a time to keep memory constant for large exports.
			$result = fputcsv( $handle, $row );
			if ( false === $result ) {
				throw new \RuntimeException(
					sprintf(
						/* translators: %s: file path. */
						__( 'Failed to write CSV row: %s', 'rt-movie-library' ),
						(string) $path
					)
				);
			}
		} finally {
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- Required counterpart to fopen CSV stream lifecycle.
			fclose( $handle );
		}
	}
