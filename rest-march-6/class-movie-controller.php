...
class Movie_Controller extends Base_Cpt_Controller {
	...
	/**
	 * Resolve attachment payload (ID + URL) or return null.
	 *
	 * @param mixed $attachment_id Attachment ID.
	...
	/**
	 * Resolve list of attachments as ID + URL payload entries.
	 *
	...
	private function attachment_payload_list( array $attachment_ids ): array {
		...
		foreach ( $attachment_ids as $attachment_id ) {
			$entry = $this->attachment_payload_or_null( $attachment_id );
	...
	private function is_exact_duplicate_for_request( int $post_id, WP_REST_Request $request ): bool {
		...
		if ( ! $this->is_core_meta_fields_identical( $post_id, $request, $has_any_comparison ) ) {
			return false;
		}
	 ...
	 * @return bool
	 */
	private function is_core_meta_fields_identical( int $post_id, WP_REST_Request $request, bool &$has_any_comparison ): bool {
