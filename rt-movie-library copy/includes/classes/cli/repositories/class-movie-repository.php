...
class Movie_Repository {
	...
	public function get_meta( int $movie_id ): array {
		...
		// Define the custom meta keys we handle in custom tables.
		$plugin_keys = array(
		...
		foreach ( $plugin_keys as $key ) {
			// Only fetch from Meta_Repository if it's missing or empty in wp_postmeta (Zero Bloat goal).
			if ( empty( $meta[ $key ] ) ) {
				$val = Meta_Repository::get( $movie_id, $key, false );
				if ( ! empty( $val ) ) {
					$meta[ $key ] = is_array( $val ) ? $val : array( $val );
				}
			}
		}
	...
	public function import_comments( int $post_id, array $comments ): void {
		...
		foreach ( $comments as $comment_data ) {
			...
			$commentarr = array(
				...
				'comment_content'      => wp_kses_post( (string) ( $comment_data['comment_content'] ?? '' ) ),
