...
class Movie_Export_Service {
	 ...
	 * @param callable|null $progress_callback Optional callback invoked per exported movie.
	 * @return array<string, mixed>
	 */
	public function export_movies( string $file_path, ?callable $progress_callback = null ): array {
		...
		$handle = $filesystem->open_write_stream( $resolved_path, false );

		try {
			$filesystem->write_csv_row_to_stream( $handle, Csv_Helper::HEADERS, $resolved_path );

			foreach ( $repository->iterate_movies() as $movie_post ) {
				$row = $this->build_row( $movie_post->ID, $repository, $transformer );

				$filesystem->write_csv_row_to_stream( $handle, $row, $resolved_path );
				++$export_count;
			...
			$filesystem->close_stream( $handle, $resolved_path );
		}
	...
	/**
	 * Count movies that will be exported.
	 *
	 * @return int
	 */
	public function count_exportable_movies(): int {
		$repository = new Movie_Repository();
		return $repository->count_movies_for_export();
	}
	...
	private function build_row( int $movie_id, Movie_Repository $repository, Movie_Cli_Transformer $transformer ): array {
		...
		$basic_rating         = Meta_Repository::get( $movie_id, 'rt-movie-meta-basic-rating', true );
		$basic_runtime        = Meta_Repository::get( $movie_id, 'rt-movie-meta-basic-runtime', true );
		$basic_release_date   = Meta_Repository::get( $movie_id, 'rt-movie-meta-basic-release-date', true );
		$basic_content_rating = Meta_Repository::get( $movie_id, 'rt-movie-meta-basic-content-rating', true );
