...
class Dashboard_Widgets {
	...
	public function render_recent_movies(): void {
		...
		foreach ( $query->posts as $post ) {
			...
			$release_date = trim( (string) Meta_Repository::get( $post->ID, self::META_RELEASE_DATE, true ) );
	...
	public function render_top_rated_movies(): void {
		...
		foreach ( $post_ids as $post_id ) {
			...
			$rating    = (float) Meta_Repository::get( $post_id, self::META_RATING, true );
	...
	public function render_upcoming_movies(): void {
		...
		if ( is_wp_error( $movies ) ) {
			Logger::error( 'Upcoming movies widget failed — ' . $movies->get_error_message(), 'dashboard' );
