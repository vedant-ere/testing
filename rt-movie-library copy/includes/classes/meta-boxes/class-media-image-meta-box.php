...
class Media_Image_Meta_Box {
	...
	public function render( WP_Post $post ): void {
		...
		$value = Meta_Repository::get( $post->ID, 'rt-media-meta-img', true );
		?>
	...
	public function save( int $post_id ): void {
		...
		if ( isset( $_POST['rt_media_img'] ) ) {
			$raw_media = wp_unslash( $_POST['rt_media_img'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in sanitize_media_ids().
			...
			if ( ! empty( $media_ids ) ) {
				...
				Meta_Repository::delete( $post_id, 'rt-media-meta-img' );
			}
		}
	}
