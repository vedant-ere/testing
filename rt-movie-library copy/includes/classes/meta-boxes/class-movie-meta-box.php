...
class Movie_Meta_Box {
	...
	public function enqueue_assets( string $hook ): void {
		...
		if ( ( 'post.php' === $hook || 'post-new.php' === $hook ) && 'rt-movie' === $post_type ) {
			...
			wp_enqueue_script(
				'rt-movie-meta-validation',
				plugin_dir_url( RT_MOVIE_LIBRARY_PATH . 'rt-movie-library.php' ) . 'assets/js/admin/movie-meta-validation.js',
				array(),
				RT_MOVIE_LIBRARY_VERSION,
				true
			);
			...
			wp_localize_script(
				...
				'rtMovieMetaL10n',
				array(
					'ratingMessage'  => __( 'Rating must be between 1.0 and 10.0', 'rt-movie-library' ),
					'runtimeMessage' => __( 'Runtime must be between 1 and 300 minutes', 'rt-movie-library' ),
				)
			);
		}
	}
	...
	public function render( WP_Post $post ): void {
		...
		$rating  = Meta_Repository::get( $post->ID, 'rt-movie-meta-basic-rating', true );
		$runtime = Meta_Repository::get( $post->ID, 'rt-movie-meta-basic-runtime', true );
	...
	<div class="rt-movie-basic-field">
		...
		<input
			...
			class="rt-movie-basic-input"
			value="<?php echo esc_attr( Meta_Repository::get( $post->ID, 'rt-movie-meta-basic-release-date', true ) ); ?>"
		>
	</div>
	...
	public function save( int $post_id ): void {
		...
		// Also skip revisions — they are snapshots, not the canonical post.
		...
		if ( isset( $_POST['rt_movie_rating'] ) ) {
			...
			if ( '' === $rating_raw || ! is_numeric( $rating_raw ) ) {
					Meta_Repository::delete( $post_id, 'rt-movie-meta-basic-rating' );
			} else {
		...
		if ( isset( $_POST['rt_movie_runtime'] ) ) {
			...
			if ( '' === $runtime_raw || ! is_numeric( $runtime_raw ) ) {
					Meta_Repository::delete( $post_id, 'rt-movie-meta-basic-runtime' );
			} else {
				$runtime = (float) $runtime_raw;

				if ( floor( $runtime ) !== $runtime ) {
					Meta_Repository::delete( $post_id, 'rt-movie-meta-basic-runtime' );
				} elseif ( 0 < $runtime && 300 >= $runtime ) {
					Meta_Repository::update(
						$post_id,
						'rt-movie-meta-basic-runtime',
						(int) $runtime
		...
		if ( isset( $_POST['rt_movie_release_date'] ) ) {
			...
			if ( '' === $release_date ) {
					Meta_Repository::delete( $post_id, 'rt-movie-meta-basic-release-date' );
