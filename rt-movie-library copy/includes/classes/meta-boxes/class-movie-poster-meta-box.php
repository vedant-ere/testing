...
class Movie_Poster_Meta_Box {
	...
	public function render( WP_Post $post ): void {
		...
		$value = Meta_Repository::get( $post->ID, 'rt-movie-meta-carousel-poster', true );
		?>
	...
	public function save( int $post_id ): void {
		...
		if ( isset( $_POST['rt_movie_poster'] ) ) {
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in is_valid_image_attachment().
			$value = (string) wp_unslash( $_POST['rt_movie_poster'] );
			...
			if ( $this->is_valid_image_attachment( $attachment_id ) ) {
				...
				Meta_Repository::delete( $post_id, 'rt-movie-meta-carousel-poster' );
			}
		}
	}
