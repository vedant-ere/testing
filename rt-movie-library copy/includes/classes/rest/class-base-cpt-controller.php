...
abstract class Base_Cpt_Controller extends WP_REST_Controller {
	...
	public function get_collection_params() {
		$statuses       = Cpt_Helper::get_public_post_statuses();
		$default_status = Cpt_Helper::get_default_public_status();
		...
		return array(
			'page'     => array(
				...
				'minimum'           => 1,
				'sanitize_callback' => 'absint',
				'validate_callback' => array( Cpt_Helper::class, 'validate_collection_page' ),
			),
			'per_page' => array(
				...
				'maximum'           => 100,
				'sanitize_callback' => 'absint',
				'validate_callback' => array( Cpt_Helper::class, 'validate_collection_per_page' ),
			),
			'search'   => array(
			...
			'status'   => array(
				...
				'default'           => array( $default_status ),
				'sanitize_callback' => array( Cpt_Helper::class, 'sanitize_status_list' ),
				'validate_callback' => array( Cpt_Helper::class, 'validate_status_list' ),
			),
		);
	}
	...
	protected function get_create_update_args( bool $required_title ): array {
		$available_statuses  = Cpt_Helper::get_public_post_statuses();
		$default_post_status = in_array( 'draft', $available_statuses, true ) ? 'draft' : Cpt_Helper::get_default_public_status();
	...
	public function get_items_permissions_check( $request ) {
		if ( ! $request instanceof WP_REST_Request ) {
			return new WP_Error( 'rt_rest_invalid_request', __( 'Invalid request.', 'rt-movie-library' ), array( 'status' => 400 ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		$value      = $request instanceof WP_REST_Request ? $request->get_param( 'id' ) : null;
		$validation = Cpt_Helper::validate_id( $value, new WP_REST_Request( 'GET' ), 'id' );
	...
	public function get_item_by_slug_permissions_check( $request ) {
		$value      = $request instanceof WP_REST_Request ? $request->get_param( 'slug' ) : null;
		$validation = Cpt_Helper::validate_slug( $value, new WP_REST_Request( 'GET' ), 'slug' );
	...
	protected function persist_all_fields( int $post_id, WP_REST_Request $request ) {
		$meta_result = $this->persist_meta_fields( $post_id, $request );
	 ...
	 * @return true|WP_Error
	 */
	protected function persist_meta_fields( int $post_id, WP_REST_Request $request ) {
		foreach ( $this->meta_field_map as $request_key => $meta_key ) {
			if ( $request->has_param( $request_key ) ) {
				$validation = $this->validate_meta_value( $meta_key, $request->get_param( $request_key ), $request );
	...
	/**
	 * Validate one meta value before persistence.
	 *
	 * Child controllers should override for resource-specific constraints.
	 *
	...
	protected function validate_meta_value( string $meta_key, $value, WP_REST_Request $request ) {
		unset( $meta_key, $value, $request );
		return true;
	}

	/**
	 * Validate request payload before create/update.
	 *
	 * @param WP_REST_Request $request Request.
	 * @return true|WP_Error
	 */
	...
	protected function upsert_meta_value( int $post_id, string $meta_key, $value ): void {
		if ( 'rt-media-meta-img' === $meta_key ) {
			...
			if ( empty( $image_ids ) ) {
				...
				Meta_Repository::update( $post_id, $meta_key, wp_json_encode( $image_ids ) );
			}
		...
		Meta_Repository::update( $post_id, $meta_key, $value );
	}
	...
	protected function get_meta_payload( int $post_id ): array {
		...
		foreach ( $this->meta_field_map as $request_key => $meta_key ) {
			$value = Meta_Repository::get( $post_id, $meta_key, true );
	...
	/**
	 * Determine whether current user can read a post in this REST context.
	 *
	 * @param WP_Post $post Post object.
	 * @return bool
	 */
	protected function can_read_post( WP_Post $post ): bool {
		$public_status = Cpt_Helper::get_default_public_status();

		if ( $public_status === (string) $post->post_status ) {
			return true;
		}
