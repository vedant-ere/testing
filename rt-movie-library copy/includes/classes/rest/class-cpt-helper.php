...
class Cpt_Helper {
	...
	public static function validate_person_social_url( $value, WP_REST_Request $request, string $param ) {
		...
		$allowed_domains_map = array(
			...
			// 'website' intentionally omitted: any valid URL is accepted.
			'website'   => array(),
		);
		...
		// Empty allowed list means any domain is accepted (e.g. 'website').
	...
	/**
	 * Return public post statuses allowed by this API.
	 *
	 * @return array<int, string>
	 */
	public static function get_public_post_statuses(): array {
		$statuses = get_post_stati( array( 'internal' => false ), 'names' );
		...
		$publish_index = array_search( 'publish', $statuses, true );
		if ( false !== $publish_index ) {
			unset( $statuses[ $publish_index ] );
	...
	/**
	 * Return the default readable status for collection filters.
	 *
	 * @return string
	 */
	public static function get_default_public_status(): string {
		$statuses = self::get_public_post_statuses();

		return in_array( 'publish', $statuses, true ) ? 'publish' : $statuses[0];
	}

	/**
	 * Validate collection page argument.
	 *
	 * @param mixed           $value   Parameter value.
	 * @param WP_REST_Request $request Full request.
	 * @param string          $param   Parameter name.
	 * @return bool|WP_Error
	 */
	public static function validate_collection_page( $value, WP_REST_Request $request, string $param ) {
		unset( $request, $param );
		...
		return new WP_Error(
			'rt_rest_invalid_page',
			__( 'Page must be an integer greater than or equal to 1.', 'rt-movie-library' ),
			array( 'status' => 400 )
		);
	}

	/**
	 * Validate collection per-page argument.
	 *
	 * @param mixed           $value   Parameter value.
	 * @param WP_REST_Request $request Full request.
	 * @param string          $param   Parameter name.
	 * @return bool|WP_Error
	 */
	public static function validate_collection_per_page( $value, WP_REST_Request $request, string $param ) {
		unset( $request, $param );
		...
		return new WP_Error(
			'rt_rest_invalid_per_page',
			__( 'Per-page must be an integer between 1 and 100.', 'rt-movie-library' ),
			array( 'status' => 400 )
		);
	}

	/**
	 * Validate collection search argument.
	 *
	 * @param mixed           $value   Parameter value.
	 * @param WP_REST_Request $request Full request.
	 * @param string          $param   Parameter name.
	 * @return bool|WP_Error
	 */
	public static function validate_collection_search( $value, WP_REST_Request $request, string $param ) {
		unset( $request, $param );
		...
		return new WP_Error(
			'rt_rest_invalid_search',
			__( 'Search must be a string.', 'rt-movie-library' ),
			array( 'status' => 400 )
		);
	}

	/**
	 * Validate collection status list argument.
	 *
	 * Returns a WP_Error that includes the list of allowed statuses so API
	 * consumers know exactly which values are accepted.
	 *
	 * @param mixed           $value   Parameter value.
	 * @param WP_REST_Request $request Full request.
	 * @param string          $param   Parameter name.
	 * @return bool|WP_Error
	 */
	public static function validate_status_list( $value, WP_REST_Request $request, string $param ) {
		unset( $request, $param );

		$allowed = self::get_public_post_statuses();

		if ( ! is_array( $value ) ) {
			return new WP_Error(
				'rt_rest_invalid_status',
				__( 'Status must be an array of post statuses.', 'rt-movie-library' ),
				array(
					'status'  => 400,
					'allowed' => $allowed,
				)
		...
		foreach ( $value as $status ) {
			if ( ! is_scalar( $status ) || ! in_array( sanitize_key( (string) $status ), $allowed, true ) ) {
				return new WP_Error(
					'rt_rest_invalid_status',
					sprintf(
						/* translators: %s: comma-separated list of valid post statuses. */
						__( 'One or more post statuses are invalid. Allowed values: %s.', 'rt-movie-library' ),
						implode( ', ', $allowed )
					),
					array(
						'status'  => 400,
	...
	public static function sanitize_status_list( $value ): array {
		$allowed = self::get_public_post_statuses();
		$default = self::get_default_public_status();
	...
	public static function find_or_create_person_reference( $person_ref, string $role = '' ): int {
		...
		$person_id = wp_insert_post(
			array(
				'post_type'   => 'rt-person',
				'post_status' => self::get_default_public_status(),
				'post_title'  => $title,
			),
			true
		);
