...
class Movie_Duplicate_Handler {
	...
	public function pick_best_candidate( array $candidates, array $incoming_sig ): ?WP_Post {
		...
		foreach ( $candidates as $candidate ) {
			...
			$release = $this->normalize_date( Meta_Repository::get( $candidate->ID, 'rt-movie-meta-basic-release-date', true ) );
			if ( ! empty( $incoming_sig['release_date'] ) && $release === $incoming_sig['release_date'] ) {
				++$score;
			}
 
			$runtime = $this->normalize_runtime( Meta_Repository::get( $candidate->ID, 'rt-movie-meta-basic-runtime', true ) );
			if ( ! empty( $incoming_sig['runtime'] ) && $runtime === $incoming_sig['runtime'] ) {
				++$score;
			}
 
			$content_rating = $this->normalize_content_rating( Meta_Repository::get( $candidate->ID, 'rt-movie-meta-basic-content-rating', true ) );
	...
	private function is_meta_value_identical( int $post_id, string $meta_key, $incoming ): bool {
		$existing = Meta_Repository::get( $post_id, $meta_key, true );
	...
	private function is_crew_payload_identical( int $post_id, $crew_payload, Movie_Controller $controller ): bool {
		...
		$existing_grouped = array(
			'director' => $controller->resolve_attachment_ids_public( Meta_Repository::get( $post_id, 'rt-movie-meta-crew-director', true ) ),
			'producer' => $controller->resolve_attachment_ids_public( Meta_Repository::get( $post_id, 'rt-movie-meta-crew-producer', true ) ),
			'writer'   => $controller->resolve_attachment_ids_public( Meta_Repository::get( $post_id, 'rt-movie-meta-crew-writer', true ) ),
			'actor'    => $controller->resolve_attachment_ids_public( Meta_Repository::get( $post_id, 'rt-movie-meta-crew-actor', true ) ),
		);
		...
		$existing_chars_raw = Meta_Repository::get( $post_id, 'rt-movie-meta-crew-actor-characters', true );
	...
	private function get_existing_director_names( int $post_id ): array {
		$raw_json = Meta_Repository::get( $post_id, 'rt-movie-meta-crew-director', true );
