...
class Tmdb_Sync {
	...
	/**
	 * Meta key storing the TMDB movie ID.
	 *
	 * @var string
	 */
	private const META_TMDB_MOVIE_ID = '_rt_tmdb_movie_id';

	/**
	 * Meta key storing the last sync error message for a post.
	 *
	 * @var string
	 */
	private const META_SYNC_ERROR = '_rt_tmdb_sync_error';
	...
	/**
	 * Delay between TMDB requests during cron sync (microseconds).
	 *
	 * @var int
	 */
	...
	public function run_sync(): void {
		try {
			...
			$client = Tmdb_Client::get_instance();
			...
			$last_index = count( $movies ) - 1;

			foreach ( $movies as $index => $post ) {
			...
			Logger::error( sanitize_text_field( $e->getMessage() ), 'tmdb-sync' );
	...
	private function sync_single_movie( \WP_Post $post, Tmdb_Client $client ): void {
		...
		if ( is_wp_error( $tmdb_data ) ) {
			$error_message = $tmdb_data->get_error_message();

			Logger::error(
				sprintf( 'Sync failed for post %d ("%s"): %s', $post->ID, $post->post_title, $error_message ),
				'tmdb-sync'
			);

			Meta_Repository::update( $post->ID, self::META_SYNC_ERROR, sanitize_text_field( $error_message ) );
			Meta_Repository::update( $post->ID, self::META_SYNCED_AT, time() );
			return;
		}

		// Clear any previous sync error on success.
		Meta_Repository::delete( $post->ID, self::META_SYNC_ERROR );

		$tmdb_id = absint( $tmdb_data['id'] ?? 0 );

		if ( $tmdb_id > 0 ) {
			Meta_Repository::update( $post->ID, self::META_TMDB_MOVIE_ID, $tmdb_id );
		}

		$new_rating      = (float) ( $tmdb_data['vote_average'] ?? 0 );
		$existing_rating = (float) Meta_Repository::get( $post->ID, 'rt-movie-meta-basic-rating', true );

		if ( abs( $existing_rating - $new_rating ) > 0.001 ) {
			Meta_Repository::update( $post->ID, 'rt-movie-meta-basic-rating', $new_rating );
		}

		$new_date      = trim( (string) ( $tmdb_data['release_date'] ?? '' ) );
		$existing_date = trim( (string) Meta_Repository::get( $post->ID, 'rt-movie-meta-basic-release-date', true ) );

		if ( '' !== $new_date && $existing_date !== $new_date ) {
			Meta_Repository::update( $post->ID, 'rt-movie-meta-basic-release-date', $new_date );
		}
	...
	private function sync_movie_poster( int $post_id, string $poster_path ): void {
		...
		$existing_path          = trim( (string) Meta_Repository::get( $post_id, self::META_POSTER_PATH, true ) );
		$existing_attachment_id = absint( Meta_Repository::get( $post_id, self::META_POSTER_ATTACHMENT_ID, true ) );
		...
		if ( is_wp_error( $attachment_id ) ) {
			Logger::error(
				sprintf( 'Poster sideload failed for post %d: %s', $post_id, $attachment_id->get_error_message() ),
				'tmdb-sync'
			);
		...
		Meta_Repository::update( $post_id, self::META_POSTER_PATH, $poster_path );
		Meta_Repository::update( $post_id, self::META_POSTER_ATTACHMENT_ID, $attachment_id );
	}
