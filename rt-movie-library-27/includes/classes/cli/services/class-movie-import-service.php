...
class Movie_Import_Service {
	...
	public function import_movies( string $file_path ): array {
		...
		if ( '' === trim( $contents ) ) {
			throw new Cli_Validation_Exception( __( 'Import file is empty or unreadable.', 'rt-movie-library' ) );
		}

		$lines = $csv_helper->split_lines( $contents );
		if ( empty( $lines ) ) {
			throw new Cli_Validation_Exception( __( 'Import file has no parsable rows.', 'rt-movie-library' ) );
		}
