...
class Rewrite_Rules {
	...
	public function register_rewrite_tags(): void {
		add_rewrite_tag( '%mw_genre%', '([^/]+)', 'mw_genre=' );
		add_rewrite_tag( '%mw_career%', '([^/]+)', 'mw_career=' );
	}
	 ...
	 * @param bool     $leavename Keep post name placeholder.
	 * @param bool     $sample    Whether this is a sample permalink.
	 * @return string
	 */
	public function filter_post_type_link( string $link, \WP_Post $post, bool $leavename = false, bool $sample = false ): string {
