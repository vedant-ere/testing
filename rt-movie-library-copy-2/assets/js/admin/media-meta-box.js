...
(() => {
	...
	function renderItem(attachment) {
		...
		const removeTemplate = (window.rtMediaBoxL10n && rtMediaBoxL10n.removeLabel) || 'Remove %s';
		const fileLabel = attachment.filename || 'media';
		removeBtn.setAttribute('aria-label', removeTemplate.replace('%s', fileLabel));
	...
	function initBox(box) {
        ...
        button.addEventListener('click', (event) => {
            ...
            if (!frame) {
                frame = wp.media({
                    title: (window.rtMediaBoxL10n && rtMediaBoxL10n.selectMedia) || 'Select media',
                    button: { text: (window.rtMediaBoxL10n && rtMediaBoxL10n.useMedia) || 'Use selected media' },
                    library: { type },
                    multiple,
                });
