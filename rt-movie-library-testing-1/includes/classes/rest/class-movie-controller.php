...
class Movie_Controller extends Base_Cpt_Controller {
	...
	public function get_items( $request ) {
		...
		// Director filtering via shadow taxonomy (aligned with PHP render callback).
		if ( $director_id > 0 ) {
	...
	protected function get_additional_response_payload( int $post_id ): array {
		$release_date_raw       = (string) get_post_meta( $post_id, 'rt-movie-meta-basic-release-date', true );
		...
		$img_ids      = $this->resolve_attachment_ids( get_post_meta( $post_id, 'rt-media-meta-img', true ) );
		$video_ids    = $this->resolve_attachment_ids( get_post_meta( $post_id, 'rt-media-meta-video', true ) );
		$carousel_raw = get_post_meta( $post_id, 'rt-movie-meta-carousel-poster', true );
