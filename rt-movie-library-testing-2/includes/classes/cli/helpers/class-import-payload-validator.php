...
// phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped -- CLI exception messages are consumed by WP_CLI output handlers, not rendered in browser HTML. esc_html() would corrupt paths/values.
...
class Import_Payload_Validator {
	...
	private function validate_taxonomy_fields( array $row, int $row_number ): void {
		...
		foreach ( $taxonomy_fields as $field_name ) {
			...
			foreach ( $decoded as $term_value ) {
				...
				$term_text = trim( sanitize_text_field( (string) $term_value ) );
				if ( '' === $term_text ) {
					throw new \RuntimeException(
						sprintf(
							/* translators: 1: CSV row number. 2: field name. */
							__( 'Skipping row %1$d: %2$s cannot contain empty taxonomy values.', 'rt-movie-library' ),
							absint( $row_number ),
							(string) $field_name
						)
					);
	...
	private function validate_comments_json( array $row, int $row_number ): void {
		...
		foreach ( $decoded as $comment ) {
			...
			$this->validate_comment_text_fields( $comment, $row_number );
			...
			$this->validate_comment_meta_payload( $meta, $row_number );
		}
	}
	...
	/**
	 * Validate comment text-ish fields are scalar values.
	 *
	 * @param array<string, mixed> $comment Comment payload.
	 * @param int                  $row_number CSV row number.
	 * @return void
	 * @throws \RuntimeException When fields are invalid.
	 */
	private function validate_comment_text_fields( array $comment, int $row_number ): void {
		$text_fields = array( 'comment_author', 'comment_content', 'comment_type' );
		...
		foreach ( $text_fields as $field_name ) {
			...
			if ( ! is_scalar( $comment[ $field_name ] ) ) {
				throw new \RuntimeException(
					sprintf(
						/* translators: 1: CSV row number. 2: field name. */
						__( 'Skipping row %1$d: %2$s must be a scalar value.', 'rt-movie-library' ),
						absint( $row_number ),
						(string) $field_name
					)
				);
	...
	/**
	 * Validate comment meta payload.
	 *
	 * @param array<string, mixed> $meta Comment meta payload.
	 * @param int                  $row_number CSV row number.
	 * @return void
	 * @throws \RuntimeException When meta payload is invalid.
	 */
	private function validate_comment_meta_payload( array $meta, int $row_number ): void {
		foreach ( $meta as $meta_key => $meta_values ) {
			...
			if ( '' === sanitize_key( $meta_key ) ) {
				throw new \RuntimeException(
					sprintf(
						/* translators: %d: CSV row number. */
						__( 'Skipping row %d: comment meta key is invalid.', 'rt-movie-library' ),
						absint( $row_number )
					)
				);
			}
