...
class Tmdb_Sync {
	...
	private function sync_single_movie( \WP_Post $post, Tmdb_Client $client ): void {
		...
		if ( is_wp_error( $tmdb_data ) ) {
			...
			update_post_meta( $post->ID, self::META_SYNC_ERROR, sanitize_text_field( $error_message ) );
			update_post_meta( $post->ID, self::META_SYNCED_AT, time() );
			return;
		}

		// Clear any previous sync error on success.
		delete_post_meta( $post->ID, self::META_SYNC_ERROR );

		$tmdb_id = absint( $tmdb_data['id'] ?? 0 );

		if ( $tmdb_id > 0 ) {
			update_post_meta( $post->ID, self::META_TMDB_MOVIE_ID, $tmdb_id );
		}
