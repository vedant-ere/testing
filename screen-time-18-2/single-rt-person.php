...
get_header();
// temporary inside single-rt-person.php
echo 'Person ID: ' . get_the_ID();
echo ' Person slug: ' . get_post_field( 'post_name', get_the_ID() );
...
<main class="page-single-person">
	...
	while ( have_posts() ) :
		...
		$post_id        = get_the_ID();
		$full_name      = (string) get_post_meta( $post_id, 'rt-person-meta-full-name', true );
		$birth_date_raw = (string) get_post_meta( $post_id, 'rt-person-meta-basic-birth-date', true );
		$birth_place    = (string) get_post_meta( $post_id, 'rt-person-meta-basic-birth-place', true );
		$careers        = screentime_get_person_career_names( $post_id );
		$social_links   = screentime_get_person_social_links( $post_id );
		$photo_gallery  = screentime_get_movie_photo_gallery( $post_id );
		$video_gallery  = screentime_get_movie_video_gallery( $post_id );
		$popular_movies = screentime_get_popular_movies_for_person( $post_id, 3 );
		$profile_image  = get_the_post_thumbnail_url( $post_id, 'large' );
		$birth_date     = '';
		...
		<section class="person-about" id="about">
			<div class="container person-about__grid">
				<article class="person-about__content">
					<h2 class="section-title"><?php esc_html_e( 'About', 'screen-time' ); ?></h2>
					<?php the_content(); ?>
				</article>
			...
			<div class="container">
				...
				<div class="person-snapshot-grid">
					<?php foreach ( $photo_gallery as $index => $snapshot ) : ?>
						<img src="<?php echo esc_url( (string) $snapshot['url'] ); ?>" alt="<?php echo esc_attr( ! empty( $snapshot['alt'] ) ? (string) $snapshot['alt'] : sprintf( __( 'Snapshot %d', 'screen-time' ), $index + 1 ) ); ?>" width="592" height="419">
					<?php endforeach; ?>
				</div>
			</div>
		</section>
		...
		<section class="person-section" id="videos">
			<div class="container">
				<h2 class="section-title"><?php esc_html_e( 'Videos', 'screen-time' ); ?></h2>
				<div class="person-video-grid">
					<?php foreach ( $video_gallery as $index => $video ) : ?>
						...
						?>
						<button type="button" class="person-video-card" data-video-url="<?php echo esc_url( (string) $video['url'] ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Play video %d', 'screen-time' ), $index + 1 ) ); ?>">
							<img src="<?php echo esc_url( $poster ); ?>" alt="<?php echo esc_attr( sprintf( __( 'Video %d', 'screen-time' ), $index + 1 ) ); ?>" width="384" height="246">
