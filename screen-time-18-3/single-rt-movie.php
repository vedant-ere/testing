...
<main class="page-single-movie">
	...
	while ( have_posts() ) :
		...
		$post_id          = get_the_ID();
		$hero_image       = screentime_get_movie_image_url( $post_id, 'large', true );
		$rating           = screentime_get_movie_rating( $post_id );
		...
		$languages        = screentime_get_movie_language_names( $post_id );
		...
		<section class="movie-single-body" id="synopsis">
			<div class="container movie-single-body__grid">
				<article class="movie-single-body__copy">
					<h2 class="section-title"><?php esc_html_e( 'Synopsis', 'screen-time' ); ?></h2>
					<?php the_content(); ?>
				</article>
		...
		get_template_part(
			...
			array(
				...
				'writers'          => $writers,
				'actors'           => $actors,
				'actor_characters' => $actor_characters,
			)
		);
		...
		<section class="movie-single-form">
			<div class="container">
				...
				<form class="movie-review-form" action="<?php echo esc_url( site_url( '/wp-comments-post.php' ) ); ?>" method="post">
