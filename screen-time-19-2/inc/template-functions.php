...
function screentime_get_movie_cast_crew_cards( $post_id ) {
	$directors        = screentime_get_movie_people_by_role( $post_id, 'rt-movie-meta-crew-director' );
	$producers        = screentime_get_movie_people_by_role( $post_id, 'rt-movie-meta-crew-producer' );
	$writers          = screentime_get_movie_people_by_role( $post_id, 'rt-movie-meta-crew-writer' );
	$actors           = screentime_get_movie_people_by_role( $post_id, 'rt-movie-meta-crew-actor' );
	$actor_characters = screentime_get_movie_actor_character_map( $post_id );

	$cards = array();

	$append_people = static function( $people, $role_label ) use ( &$cards ) {
		foreach ( $people as $person ) {
			$person_id = (int) $person->ID;

			if ( ! isset( $cards[ $person_id ] ) ) {
			...
			if ( '' !== $role_label && ! in_array( $role_label, $cards[ $person_id ]['roles'], true ) ) {
				$cards[ $person_id ]['roles'][] = $role_label;
			}
		}
	};

	$append_people( $directors, __( 'Director', 'screen-time' ) );
	$append_people( $producers, __( 'Producer', 'screen-time' ) );
	$append_people( $writers, __( 'Writer', 'screen-time' ) );
	$append_people( $actors, '' );

	foreach ( $cards as $person_id => $card ) {
		if ( isset( $actor_characters[ $person_id ] ) && '' !== $actor_characters[ $person_id ] ) {
			$cards[ $person_id ]['role'] = (string) $actor_characters[ $person_id ];
			continue;
		}

		$role_labels = isset( $card['roles'] ) && is_array( $card['roles'] ) ? $card['roles'] : array();
		$cards[ $person_id ]['role'] = ! empty( $role_labels ) ? implode( ', ', $role_labels ) : '';
	}
