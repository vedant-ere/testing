		...
		<section class="movie-single-form">
			<div class="container">
				...
				<form class="movie-review-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
					...
					<h2><?php esc_html_e( 'Leave a Review', 'screen-time' ); ?></h2>
					<p><?php esc_html_e( 'Your Email Address will not be published. Required fields are marked *', 'screen-time' ); ?></p>
					<label for="review-comment"><?php esc_html_e( 'Comment*', 'screen-time' ); ?></label>
					<textarea id="review-comment" name="comment" rows="6" required="required"></textarea>
					<div class="movie-review-form__row">
						<div><label for="review-name"><?php esc_html_e( 'Name*', 'screen-time' ); ?></label><input id="review-name" name="author" type="text" value="<?php echo esc_attr( $commenter['comment_author'] ); ?>" required="required"></div>
						<div><label for="review-email"><?php esc_html_e( 'Email*', 'screen-time' ); ?></label><input id="review-email" name="email" type="email" value="<?php echo esc_attr( $commenter['comment_author_email'] ); ?>" required="required"></div>
					</div>
					<label for="review-website"><?php esc_html_e( 'Website', 'screen-time' ); ?></label>
					<input id="review-website" name="url" type="text" value="<?php echo esc_attr( $commenter['comment_author_url'] ); ?>">
					<label class="movie-review-form__checkbox"><input type="checkbox" name="wp-comment-cookies-consent" value="yes" <?php checked( ! empty( $commenter['comment_author_email'] ) ); ?>> <?php esc_html_e( 'Save my name and email in this browser for the next time I comment.', 'screen-time' ); ?></label>
					<?php comment_id_fields( $movie_post_id ); ?>
					<button type="submit"><?php esc_html_e( 'Post Review', 'screen-time' ); ?></button>
				</form>
			</div>
		</section>
