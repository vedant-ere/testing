...
$person_image = isset( $args['image'] ) ? (string) $args['image'] : '';
$person_link  = isset( $args['link'] ) ? $args['link'] : '#';
$person_class = isset( $args['class'] ) ? (string) $args['class'] : 'person-card';

if ( '' !== $person_image && 0 !== strpos( $person_image, 'http://' ) && 0 !== strpos( $person_image, 'https://' ) && 0 !== strpos( $person_image, '/' ) ) {
...
<article class="<?php echo esc_attr( $person_class ); ?>">
	<a
		...
		href="<?php echo esc_url( $person_link ); ?>"
		aria-label="<?php echo esc_attr( $person_name ); ?>"
	></a>
	<?php
