	...
	>
		<div class="hero-slider__track">
			...
			while ( $slider_query->have_posts() ) :
				...
				$image_url      = screentime_get_movie_image_url( $movie_post_id, 'full', true );
				$movie_title    = get_the_title();
				$description    = screentime_get_movie_summary( $movie_post_id );
				...
				$runtime_label  = screentime_get_movie_runtime_compact_label( $movie_post_id );
				$genre_terms    = get_the_terms( $movie_post_id, 'rt-movie-genre' );
				$genre_terms    = ( is_wp_error( $genre_terms ) || empty( $genre_terms ) ) ? array() : array_slice( $genre_terms, 0, 3 );
				?>
				<article
					...
					<div class="hero-slider__image">
						<?php if ( ! empty( $image_url ) ) : ?>
							<img
								...
								alt="<?php echo esc_attr( $movie_title ); ?>"
								width="1440"
								height="616"
							...
							<?php if ( ! empty( $genre_terms ) ) : ?>
								<div class="hero-slider__tags">
									<?php foreach ( $genre_terms as $genre_term ) : ?>
										<?php $genre_link = get_term_link( $genre_term ); ?>
										<a
											class="chip chip--outline chip--sm"
											href="<?php echo esc_url( ! is_wp_error( $genre_link ) ? $genre_link : '#' ); ?>"
										>
			...
			<div
				...
				>
					<?php for ( $index = 0; $index < $slide_index; $index++ ) : ?>
						...
						$slide_label = sprintf( __( 'Go to slide %d', 'screen-time' ), $index + 1 );
						?>
						<button
