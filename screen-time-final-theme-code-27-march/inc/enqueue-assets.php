...
function screentime_enqueue_assets() {
	...
	if ( is_singular( 'rt-movie' ) ) {
		...
		wp_enqueue_script(
			'screentime-reviews-load-more',
			SCREENTIME_URI . '/assets/js/reviews-load-more.js',
			array(),
			screentime_asset_version( '/assets/js/reviews-load-more.js' ),
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'   => wp_create_nonce( 'screentime_load_reviews' ),
				'i18n'    => array(
					'loading'  => __( 'Loading...', 'screen-time' ),
					'loadMore' => __( 'Load more reviews', 'screen-time' ),
				),
			)
		);
	}
	...
	// Post navigation component — loaded on all singular pages.
	if ( is_singular() ) {
		wp_enqueue_style(
			'screentime-component-post-navigation',
			SCREENTIME_URI . '/assets/css/components/post-navigation.css',
			array( 'screentime-global' ),
			screentime_asset_version( '/assets/css/components/post-navigation.css' )
		);
	}
	...
	if ( is_post_type_archive( 'rt-movie' ) || is_tax( 'rt-movie-genre' ) ) {
		$archive_context = is_tax( 'rt-movie-genre' ) ? 'genre' : 'post-type';
		$term_slug       = '';

		if ( 'genre' === $archive_context ) {
		...
		wp_localize_script(
			'screentime-main',
			'screenTimeMovieArchive',
			array(
				'endpoint' => rest_url( 'screentime/v1/movie-archive' ),
